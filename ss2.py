    ...
    def calendar_menu():
        ...
        def display_items():
            ...
            for date, pills in prescriptions.items():
                ...
                pill_event = "\n".join([pill.split()[0] for pill in pills])  # Display only pill name
...
# Function to display the intro screen with the WellNest name and animated heart
def display_intro():
    # Create overlay for intro
    intro_overlay = tk.Frame(root, bg="black")
    intro_overlay.place(relx=0, rely=0, relwidth=1, relheight=1)

    # Display WellNest name in red
    label = tk.Label(intro_overlay, text="WellNest", font=("Helvetica", 36), fg="red", bg="black")
    label.pack(pady=50)

    # Display and animate a heart at the bottom
    heart = tk.Label(intro_overlay, text="♥", font=("Helvetica", 72), fg="red", bg="black")
    heart.pack(pady=20)

    # Animate the heart to grow and shrink
    def animate_heart():
        for i in range(1, 3):
            heart.config(font=("Helvetica", 72 + (i * 4)))
        ...
        root.after(1000, fade_out_intro)  # Start fading out after animation ends

    def fade_out_intro():
        # Gradually fade out the intro overlay
        for i in range(100, -1, -5):
            intro_overlay.config(bg=f"#{i:02x}{i:02x}{i:02x}")  # Change to a lighter shade of black
            heart.config(fg=f"#{i:02x}{i:02x}{i:02x}")
            label.config(fg=f"#{i:02x}{i:02x}{i:02x}")
            root.update()
            root.after(50)
        intro_overlay.destroy()  # Remove the intro overlay after fading out
...
def show_main_ui():
    ...
    tk.Button(root, text="Prescription", command=open_prescription).pack(pady=30)
    tk.Button(root, text="Appointments", command=open_appointments).pack(pady=30)
    tk.Button(root, text="Close", command=close_app).pack(pady=30)
...
# Show the main UI after intro fades out
root.after(6000, show_main_ui)  # Show UI after 6 seconds
