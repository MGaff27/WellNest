...
def update_prescription_status():
    ...
    # Step 3: Find the Prescription Entry
    for entry in prescriptions[prescription_name]["Prescription"]:
        if (entry["Day"] == date.day and
                entry["Month"] == date.month and
                entry["Year"] == date.year):

            # Create a new Toplevel window for status update with checkboxes
            status_window = Toplevel()
            status_window.title("Update Status")

            # Variables to store checkbox values
            taken_on_time_var = IntVar()
            missed_var = IntVar()

            # Checkbuttons
            taken_checkbox = Checkbutton(
                status_window, text="Taken on Time", variable=taken_on_time_var)
            missed_checkbox = Checkbutton(
                status_window, text="Missed", variable=missed_var)

            taken_checkbox.pack(padx=10, pady=5)
            missed_checkbox.pack(padx=10, pady=5)

            # Function to handle checkbox selection
            def on_submit():
                if taken_on_time_var.get() == 1 and missed_var.get() == 1:
                    messagebox.showerror("Error", "Please select only one status.")
                elif taken_on_time_var.get() == 1:
                    entry["Status"] = "taken on time"
                    save_prescriptions(prescriptions)
                    messagebox.showinfo("Success", f"Status for {prescription_date} updated to 'taken on time'.")
                    status_window.destroy()
                elif missed_var.get() == 1:
                    entry["Status"] = "missed"
                    save_prescriptions(prescriptions)
                    messagebox.showinfo("Success", f"Status for {prescription_date} updated to 'missed'.")
                    status_window.destroy()
                else:
                    messagebox.showerror("Error", "Please select a status.")

            # Submit Button
            submit_button = tk.Button(status_window, text="Update Status", command=on_submit)
...
def find_medication_by_date(cal, prescription_data):
    ...
    if found_prescriptions:
        messagebox.showinfo(f"Medication for {formatted_date}", "".join(found_prescriptions))
    else:
